...
export default function BackupManager({ tasks, onImport }: Props) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-zinc-800/70 bg-zinc-900/40 px-4 py-2.5">
        <span className="flex items-center gap-1.5 text-xs font-medium text-zinc-500">
          <DatabaseBackup className="h-4 w-4" /> Резервное копирование
        </span>
        <button
          onClick={() => exportBackup(tasks)}
          className="flex items-center gap-1.5 rounded-lg border border-zinc-800 px-3 py-1.5 text-xs font-medium text-zinc-300 transition hover:border-emerald-500/50 hover:text-emerald-400"
        >
